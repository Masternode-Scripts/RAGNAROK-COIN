...
function compile_node() {
  echo -e "THIS SCRIPT IS DEVELOPED BY UDAYDEEP - Discord Name : Linux-Master : ID : #0974"
  echo -e "DOWNLOADING & INSTALLING $COIN_NAME"
  cd $TMP_FOLDER
  wget -q $COIN_REPO
  compile_error
...
ExecStop=-$COIN_DAEMON -conf=$CONFIGFOLDER/$CONFIG_FILE -datadir=$CONFIGFOLDER stop
...
function create_key() {
  echo -e "Enter your ${RED}$COIN_NAME Masternode Private Key${NC}. Generate a new ${RED}$COIN_NAME Masternode Private Key${NC} From Your Local Windows Wallet and Paste Here:"
  read -e COINKEY
  if [[ -z "$COINKEY" ]]; then
  $COIN_DAEMON -daemon
  sleep 30
...
echo -e "Configuring system to install ${YELLOW}$COIN_NAME${NC} masternode. Please wait, it may take some time for the complete installation to get finished."
...
function important_information() {
 ...
 echo -e "Check if $COIN_NAME is running by using the following command: ${RED}systemctl status RAGNAROK.service${NC}"
 echo -e "================================================================================================================================"
}
